...
app.delete('/api/persons/:id', (request, response, next) => {
  deletePersonById(request.params.id).then(() => response.status(204).end()).catch(error => next(error))
})
...
app.post('/api/persons', (request, response, next) => {
  ...
  addPerson(person).then(addedPerson => response.json(addedPerson)).catch(error => next(error))
})
